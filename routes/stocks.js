...
router.get('/list/:id', authentication, async function(req, res, next) {
    try {
        ...
        for (stock of watchlist.stocks) {   
            let response = await axios.get('https://finnhub.io/api/v1/quote?symbol=' + stock + '&token=' + process.env.api_key_token);
            ...
            let moreDataResponse = await axios.get('https://finnhub.io/api/v1/stock/metric?symbol=' + stock + '&token=' + process.env.api_key_token);
